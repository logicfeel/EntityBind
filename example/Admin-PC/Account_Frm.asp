            ...
            <div class="mOption" style="display: block;">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            <input type="text" id="m-adm_id" name="adm_id" value="" class="fText" style="width:200px;" />
                        </td>
                        <th scope="row">이름 <strong class="icoRequired">필수</strong></th>
                        <td>
                            <input type="text" id="m-admName" name="admName" value="" class="fText" style="width:200px;" />
                        </td>                        
                    </tr>
                    <tr>
                        <th scope="row">비밀번호 <strong class="icoRequired">필수</strong></th>
                        <td>
                            <input type="password" id="m-passwd" name="passwd" value="" class="fText" style="width:200px;" />
                        </td>
                        <th scope="row">사용유무 </th>
                        <td>
                            <input type="radio" name="m-use_yn" value="Y" class="fChk" checked/> 사용</label> 
                            <input type="radio" name="m-use_yn" value="N" class="fChk" /> 중지</label> 
                        </td>                        
                    </tr>                                        
                    <!--
                    <tr>
                        <th scope="row">메모</th>
                        <td colspan="3">
                        	<input type="text" id="m-memo" name="memo" value="" class="fText" style="width:600px;" />
                        </td>
                    </tr>
                    -->
                    </tbody>
...
<script src="/Admin/adm_mod/STO/Service/store-account-svc.js?<%=g_iRandomID%>"></script>
<script>
	// #######################################################################################################
	var faq = new _W.BindModelAjax(new StoreAccountService());

    this.isLog = true;  // 디버깅 모드
	
	// 속성 설정
	faq.prop["__listUrl"] = "Account_Lst.asp";
	...
	$(document).ready(function () {
        ...
        if (faq.prop["__mode"] === "EDIT") {
            setEditMode(true);
            faq.items["acc_idx"].value  = getParamsToJSON(location.href).acc_idx;
            faq.fn.procRead();
        }  else {
            setEditMode(false);
        }
