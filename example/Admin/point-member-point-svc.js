(function(global) {
    ...
    // 헬퍼 등록
    Handlebars.registerHelper('plus_value', function (p_minus_yn, p_point) {
        if (p_minus_yn === "N") return p_point + "점";
        else return "";
    });
    Handlebars.registerHelper('minus_value', function (p_minus_yn, p_point) {
        if (p_minus_yn === "Y") return p_point + "점";
        else return "";
    ...
    var PointMemberPointService  = (function (_super) {
         ...
         */
        function PointMemberPointService(p_suffix) {
            _super.call(this);
             ...
             * @type {String}
             */
            this.baseUrl = '/Admin/adm_mod/POT/MemberPoint.C.asp';
            ...
            this.prop = {
                ...
                _temp_pot:      { selector: { key: '#s-temp-pot'+ _SUFF,            type: 'html' } },
                _area_pot:      { selector: { key: 'select[name=m-pot_id'+_SUFF+']',type: 'html' } },
                ...
                sort_cd:        '',
                mp_idx:         { isNotNull: true, },
                pot_id:         { 
                    selector: { key: 'select[name=m-pot_id'+_SUFF+']',              type: 'value' },
                    isNotNull: true,
                },
                meb_idx:        { 
                    selector: { key: '#m-meb_idx'+ _SUFF,                           type: 'value' },
                    isNotNull: true,
                },
                point_it:       { 
                ...
                minus_yn:       { 
                    selector: { key: 'input[name=m-minus_yn'+_SUFF+'][type=radio]', type: 'none' },
                    setFilter: function (val) { 
                        $('input[name=m-minus_yn'+_SUFF+'][value='+ val + ']').prop('checked', true);
                    },
                    getFilter: function (val) {
                        return $('input[name=m-minus_yn'+_SUFF+']:checked').val();
                    },
                    isNotNull: true,
                },
                ord_id:         '',
                caption:        { selector: { key: '#m-caption'+ _SUFF,             type: 'value' } },
            };
            ...
            this.command = {
                ...
                list_master: {
                    url: '/Admin/adm_mod/POT/Point_Master.C.asp',
                    outputOption: 1,
                    onExecute: function(p_bindCommand) { _this.bindModel.items['cmd'].value = 'LIST'; },
                    cbOutput: function(p_result) {
                        if (global.isLog) console.log("[Service] list_master.cbOutput() : 목록출력");
                        
                        var entity = p_result['table'];

                        if (_tempPot === null) {
                            _tempPot = Handlebars.compile( _this.bindModel.items['_temp_pot'].value ); 
                        }
                        _this.bindModel.items['_area_pot'].value   = _tempPot(entity);
                    },
                    cbEnd: function(p_result) {
                        if (p_result['return'] < 0) return alert('목록조회 처리가 실패 하였습니다. Code : ' + p_result['return']);
                    }
            ...
            this.mapping = {
                ...
                sort_cd:        { list:     'bind' },
                mp_idx:         { read:     'bind' },
                pot_id:         { read:     'output',   create:     ['valid', 'bind'],  list:     'bind' },
                meb_idx:        { read:     'output',   create:     ['valid', 'bind'],  },
                point_it:       { read:     'output',   create:     ['valid', 'bind'],  },
                minus_yn:       { read:     'output',   create:     ['valid', 'bind'],  list:     'bind' },
                ord_id:         { read:     'output',   create:     'bind' },
                caption:        { read:     'output',   create:     'bind' },
            };
            ...
            this.fn = {
                ...
                moveEdit: function(p_mp_idx) {
                    var url = _this.bindModel.prop['__formUrl'];
                    location.href = url +'?mode=EDIT&mp_idx='+ p_mp_idx;
                },
                moveForm: function() {
                    var url = _this.bindModel.prop['__formUrl'];
                    location.href = url;
                },
                procRead: function (p_mp_idx) { 
                    _this.bindModel.items['mp_idx'].value = ParamGet2JSON(location.href).mp_idx;
                ...
                procPotList: function () { 
                    _this.bindModel.list_master.execute(); 
                },
            };
        }
        util.inherits(PointMemberPointService, _super);
    
        /**
         * 전처리 :: 등록  (데코레이션 패턴 : 상위 메소드 호출함)
         * @param {BindModelAjax} p_bindModel 서비스 소유자
         */
        PointMemberPointService.prototype.preRegister = function(p_bindModel) {
        ...
        return PointMemberPointService;
    ...
    if (typeof module === 'object' && typeof module.exports === 'object') {     
        ...
        global.PointMemberPointService = PointMemberPointService;
        global.page = page;
    }
