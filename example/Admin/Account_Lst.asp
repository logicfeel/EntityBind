...
'
'   확인
'   http://rtwgs4.cafe24.com/Admin/pc/mod/bod/account_Lst.asp
'
'******************************************************************************
%>
	        ...
	        <table border="1" summary="" class="eChkColor">
                ...
                <script id="s-temp-list" type="text/x-handlebars-template">
                    ...
                    <tr class="">
                        ...
                        <td>{{use_yn}}</td>
                        <td>{{create_dt}}</td>
                        <td><a href="javascript:acc.fn.moveEdit('{{acc_idx}}');" class='btnNormal'><span> 조회</span></a></td>
                    </tr>
                    {{/rows}} 
                </script>    
...
<script>
	// #######################################################################################################
	var acc = new _W.BindModelAjax(new StoreAccountService());
	
    this.isLog = true;  // 디버깅 모드

	// 속성 설정
	acc.prop["__formUrl"] = "Account_Frm.asp";
	acc.prop['__isGetLoad'] = false;

	// 이벤트 바인딩
	$('#btn_Search').click(acc.fn.searchList);
	$('#sel_Pagesize').change(acc.fn.changePagesize);
	$('#btn_Reset').click(acc.fn.resetForm);
	$('#btn_Insert').click(acc.fn.moveForm);
    //--------------------------------------------------------------
