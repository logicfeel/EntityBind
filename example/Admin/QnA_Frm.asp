...
<script>
    ...
    qna.prop["__mode"] = getParamsToJSON(location.href).mode;
	...
	// 이벤트 바인딩 : qna
    // $('#btn_Insert').click(qna.fn.procCreate);  // 등록
	$('#btn_Update').click(qna.fn.procUpdate);  // 수정
	$('#btn_Delete').click(qna.fn.procDelete);  // 삭제
	$('#btn_List').click(qna.fn.moveList);      // 목록 이동
	$('#btn_Reset').click(qna.fn.resetForm);    // 입력폼 초기화
    //--------------------------------------------------------------
	$(document).ready(function () {
        ...
        if (qna.prop["__mode"] === "EDIT") {
            setEditMode(true);
            qna.items["qna_idx"].value  = getParamsToJSON(location.href).qna_idx;
            qna.fn.procRead();
        }  else {
            setEditMode(false);
        }
