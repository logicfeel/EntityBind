...
<style>
    ...
    .layerPopup<%=pop_idx%> .layerBox {position:fixed; z-index: 1000; left:50%; top:50%; transform:translate(-50%, -50%); padding:30px; background:#fff; border-radius:6px;}

    .layerPopup<%=pop_idx%> .layerBox .title {margin-bottom:10px; padding-bottom:10px; font-weight:600; border-bottom:1px solid #d9d9d9;}
    .layerPopup<%=pop_idx%> .layerBox .cont {margin-bottom:40px;}
    .layerPopup<%=pop_idx%> .layerBox p {line-height:20px; font-size:13px;}

    .layerPopup<%=pop_idx%> .layerBox .btnClose {display:inline-block; position:absolute; right:30px; bottom:25px; padding:6px 12px; color:#444; font-size:12px; text-decoration:underline;}
    .layerPopup<%=pop_idx%> .layerBox .btnTodayHide {font-size:13px; font-weight:600; text-decoration:underline;}
</style>
<!-- layer popup content -->
