...
<!--#include virtual="/Module/POT/POT_Member.Cls.asp"-->
<%
    ...
    Dim cMember  : Set cMember = New POT_Member_Cls
    ...
    ElseIf cmd = "LIST" Then
        ...
        If Len(meb_idx) > 0 Then
            ...
            cMember.Meb_idx        = meb_idx
            ' 프로퍼티 <선택>
            If Len(ord_id) > 0      Then cMember.Ord_id          = ord_id
            If Len(orderTel) > 0    Then cMember.OrderTel        = orderTel
            If Len(orderName) > 0   Then cMember.OrderName      = orderName
            If Len(page_size) > 0   Then cMember.Page_size      = page_size
            If Len(page_count) > 0  Then cMember.Page_count     = page_count
            If Len(sort_cd) > 0     Then cMember.Sort_cd        = sort_cd
            ' 프로퍼티 <옵션>
            cMember.MsgCode        = iMsgCode

            Select Case UCase(doctype)
                Case "XML"
                    strContent = cMember.ListXml(iReturn, iRowTotal)
                Case "JSON"
                    strContent = cMember.ListJson(iReturn, iRowTotal)
                Case "DIC"
                    Set oDic = cMember.ListDic(iReturn, iRowTotal)        
                Case else
                    Set oRs = cMember.List(iReturn, iRowTotal)
            End Select
        Else
            iReturn = iMsgCode
        End If 
