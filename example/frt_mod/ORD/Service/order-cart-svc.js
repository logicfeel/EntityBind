        ...
        function OrderCartService(p_suffix) {
                ...
                read_baseDeli:       {
                    ...
                    cbEnd: function(p_entity) {
                        ...
                        if (p_entity['return'] < 0) {
                            _this.bindModel.sync.read_baseDeli.reject(new Error('조회 처리가 실패 하였습니다. Code : ' + p_entity['return']));
                        } else {
                            _this.bindModel.sync.read_baseDeli.resolve(p_entity);
                        }
             ...
             */
             this.fn = {
                ...
                procList: function() { 
                    var BASE = _this.bindModel.sync.read_baseDeli;
                    // 동기화
                    $.when(BASE).done(function (rBase) {
                        // 기준택배비 가져온 후 리스트 조회
