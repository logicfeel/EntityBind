...
<%
    ...
    If cmd = "OVER" Then
        ...
        Else
            iReturn = iMsgCode
        End If
    ElseIf cmd = "FIND_ID" Then

        iMsgCode = -2000
        ' 필수값 검사
        If Len(mebName) > 0  and Len(email) > 0  Then
            ' 프로퍼티 <필수>
            cAccount.MebName        = mebName
            cAccount.email          = email
            ' 프로퍼티 <옵션>
            cAccount.MsgCode        = iMsgCode

            Select Case UCase(doctype)
                Case "XML"
                    strContent = cAccount.FindIdXml(iReturn)
                Case "JSON"
                    strContent = cAccount.FindIdJson(iReturn)
                Case "DIC"
                    Set oDic = cAccount.FindIdDic(iReturn)        
                Case else
                    Set oRs = cAccount.FindId(iReturn)
            End Select
        Else
            iReturn = iMsgCode
        End If    
    ElseIf cmd = "FIND_PW" Then
        ...
        If Len(meb_id) > 0 Then
            ...
            Select Case UCase(doctype)
                Case "XML"
                    strContent = cAccount.FindPasswdXml(iReturn)
                Case "JSON"
                    strContent = cAccount.FindPasswdJson(iReturn)
                Case "DIC"
                    Set oDic = cAccount.FindPasswdDic(iReturn)        
                Case else
                    Set oRs = cAccount.FindPasswd(iReturn)
            End Select
        Else
            iReturn = iMsgCode
        End If    
