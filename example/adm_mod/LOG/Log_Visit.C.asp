...
<%
    ...
    If cmd = "STATS" Then
        ...
        If True Then
            ' 프로퍼티 <선택>
            If Len(keyword) > 0     Then cVisit.Keyword        = keyword
            If Len(page_size) > 0   Then cVisit.Page_size      = page_size
            If Len(page_count) > 0  Then cVisit.Page_count     = page_count
            If Len(sort_cd) > 0     Then cVisit.Sort_cd        = sort_cd
            ' 프로퍼티 <옵션>
            cVisit.MsgCode        = iMsgCode

            Select Case UCase(doctype)
                Case "XML"
                    strContent = cVisit.ListStatsXml(iReturn, iRowTotal)
                Case "JSON"
                    strContent = cVisit.ListStatsJson(iReturn, iRowTotal)
                Case "DIC"
                    Set oDic = cVisit.ListStatsDic(iReturn, iRowTotal)        
                Case else
                    Set oRs = cVisit.ListStats(iReturn, iRowTotal)
            End Select
        Else
            iReturn = iMsgCode
        End If      
    ...
    ElseIf cmd = "LIST" Then
        ...
        If True Then
            ...
            Select Case UCase(doctype)
                Case "XML"
                    strContent = cVisit.ListXml(iReturn, iRowTotal)
                Case "JSON"
                    strContent = cVisit.ListJson(iReturn, iRowTotal)
                Case "DIC"
                    Set oDic = cVisit.ListDic(iReturn, iRowTotal)        
                Case else
                    Set oRs = cVisit.List(iReturn, iRowTotal)
            End Select
        Else
            iReturn = iMsgCode
        End If      
