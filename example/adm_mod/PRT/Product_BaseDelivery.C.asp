...
<%
    ...
    ElseIf cmd = "UPDATE" Then
        ...
        If Len(sto_id) > 0 Then
            ...
            cDelivery.Sto_id      = sto_id
            ' 프로퍼티 <선택>
            If Len(deli_mn) > 0         Then cDelivery.Deli_mn  = deli_mn
            If Len(base_mn) > 0         Then cDelivery.Base_mn  = base_mn
            If Len(base_cd) > 0         Then cDelivery.Base_cd  = base_cd
            ...
            Select Case UCase(doctype)
                Case "XML"
                    strContent = cDelivery.UpdateXml(iReturn)
                Case "JSON"
                    strContent = cDelivery.UpdateJson(iReturn)
                Case "DIC"
                    Set oDic = cDelivery.UpdateDic(iReturn)        
                Case else
                    Set oRs = cDelivery.Update(iReturn)
            End Select
        Else
            iReturn = iMsgCode
        End If
