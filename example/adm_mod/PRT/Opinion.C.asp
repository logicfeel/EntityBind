...
<%
    ...
    If cmd = "CREATE" Then
        ...
        iMsgCode = -1000
        ' 필수값 검사
        If Len(prt_id) > 0 and Len(grade_cd) > 0 Then
            ' 프로퍼티 <필수>
            ...
            Case "XML"
                strContent = cOpinion.CreateXml(iReturn)
            Case "JSON"
                strContent = cOpinion.CreateJson(iReturn)
            Case "DIC"
                Set oDic = cOpinion.CreateDic(iReturn)        
            Case else
                Set oRs = cOpinion.Create(iReturn)
            End Select        
        Else
            iReturn = iMsgCode
        End If
    ElseIf cmd = "READ" Then
        ...
        iMsgCode = -2000
        ' 필수값 검사
        If Len(prt_id) > 0 and Len(opi_idx) > 0 Then
            ' 프로퍼티 <필수>
            cOpinion.Prt_id             = prt_id
            ...
            Select Case UCase(doctype)
            Case "XML"
                strContent = cOpinion.ReadXml(iReturn)
            Case "JSON"
                strContent = cOpinion.ReadJson(iReturn)
            Case "DIC"
                Set oDic = cOpinion.ReadDic(iReturn)        
            Case else
                Set oRs = cOpinion.Read(iReturn)
            End Select
        Else
            iReturn = iMsgCode
        End If    
