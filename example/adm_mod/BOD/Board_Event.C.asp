...
<%
    ...
    If cmd = "CREATE" Then
        ...
        If Len(title) > 0  Then
            ...
            cEvent.Title      = title
            ' 프로퍼티 <선택>
            If Len(writer) > 0          Then cEvent.Writer  = writer
            If Len(begin_dt) > 0        Then cEvent.Begin_dt  = begin_dt
            If Len(close_dt) > 0        Then cEvent.Close_dt  = close_dt
            If Len(contents) > 0        Then cEvent.Contents  = contents
            If Len(active_yn) > 0       Then cEvent.Active_yn  = active_yn
            ' 프로퍼티 <옵션>
            cEvent.MsgCode        = iMsgCode

            Select Case UCase(doctype)
                Case "XML"
                    strContent = cEvent.CreateXml(iReturn)
                Case "JSON"
                    strContent = cEvent.CreateJson(iReturn)
                Case "DIC"
                    Set oDic = cEvent.CreateDic(iReturn)        
                Case else
                    Set oRs = cEvent.Create(iReturn)
            End Select        
        Else
            iReturn = iMsgCode
        End If
    ...
    ElseIf cmd = "UPDATE" Then
        ...
        If Len(evt_idx) > 0  Then
            ...
            cEvent.Evt_idx      = evt_idx
            ' 프로퍼티 <선택>
            If Len(title) > 0           Then cEvent.Title  = title
            If Len(writer) > 0          Then cEvent.Writer  = writer
            If Len(begin_dt) > 0        Then cEvent.Begin_dt  = begin_dt
            If Len(close_dt) > 0        Then cEvent.Close_dt  = close_dt
            If Len(contents) > 0        Then cEvent.Contents  = contents
            ...
            Select Case UCase(doctype)
                Case "XML"
                    strContent = cEvent.UpdateXml(iReturn)
                Case "JSON"
                    strContent = cEvent.UpdateJson(iReturn)
                Case "DIC"
                    Set oDic = cEvent.UpdateDic(iReturn)        
                Case else
                    Set oRs = cEvent.Update(iReturn)
            End Select
        Else
            iReturn = iMsgCode
        End If
    ElseIf cmd = "DELETE" Then
        ...
        If Len(evt_idx) > 0  Then
            ...
            Select Case UCase(doctype)
                Case "XML"
                    strContent = cEvent.DeleteXml(iReturn)
                Case "JSON"
                    strContent = cEvent.DeleteJson(iReturn)
                Case "DIC"
                    Set oDic = cEvent.DeleteDic(iReturn)        
                Case else
                    Set oRs = cEvent.Delete(iReturn)
            End Select
        Else
            iReturn = iMsgCode
        End If      
