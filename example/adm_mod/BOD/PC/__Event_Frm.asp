...
<script src="/Admin/adm_mod/BOD/Service/_board-event-svc.js?<%=g_iRandomID%>"></script>
<script src="/Admin/adm_mod/SYS/Service/system-image-svc.js?<%=g_iRandomID%>"></script>
<script>
    //--------------------------------------------------------------
	// 1. 생성 및 서비스 설정
    var event              = new BindModelAjax();
    var image              = new BindModelAjax();
    
    event.setService(new BoardEventService(event));
    image.setService(new SystemImageService(image));
	//--------------------------------------------------------------    
	// 2. 객체 설정 (등록)
	event.prop["__listUrl"] 	        = "Event_Lst.asp";
    
    image.items["position_cd"].value 	= "Event";
    image.items["prefix"].value 	    = "EVT-";
    //--------------------------------------------------------------    
	// 6. 외부 호출 함수 구현
	// function view(p_evt_idx) {
	// 	var url = event.prop["__formUrl"];
	// 	location.href = url + "?mode=EDIT&evt_idx=" + p_evt_idx;
    // }
    event.create.onExecute = function(p_bindCommand) { 
        oEditors.getById["m-contents"].exec("UPDATE_CONTENTS_FIELD", []);   // nhn 웹데이터

    };
    event.update.onExecute = function(p_bindCommand) { 
        oEditors.getById["m-contents"].exec("UPDATE_CONTENTS_FIELD", []);   // nhn 웹데이터
    };
    event.read.onExecuted = function(p_bindCommand, p_result) { 
        image.items["pos_idx"].value   = event.items["evt_idx"].value;      // image 키 등록
        image.list.execute();
    };
