...
<div style="width:1020px">   
	...
	<div class="section" id="QA_level2">
	    <div class="mState">
	        ...
	        <div class="gRight">
	            <select id="s-txt-pageSize-opinion" name="listCnt" class="fSelect">
	        ...
	        <table border="1" summary="" class="eChkColor">
                ...
                <script id="s-temp-list" type="text/x-handlebars-template">
                    ...
                    <tr class="">
						...
						<td>{{type_cd}}</td>
                        <td>{{active_yn}}</td>
                        <td><a href="javascript:view('{{pop_idx}}');" class='btnNormal'><span> 조회</span></a></td>
                    </tr>
                    {{/rows}} 
                </script>           
...
<script>
	...
	//--------------------------------------------------------------    
	// 2. 객체 설정 (등록)
    popup.prop["__isGetLoad"] 	= false;
	popup.prop["__formUrl"] 	= "Popup_Frm.asp";
    //--------------------------------------------------------------    
	// 6. 외부 호출 함수 구현
	function view(p_idx) {
		var url = popup.prop["__formUrl"];
		location.href = url + "?mode=EDIT&pop_idx=" + p_idx;
    }
	function createFrom() {
		var url = popup.prop["__formUrl"];
