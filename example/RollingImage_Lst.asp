...
<div style="width:1020px">   
	...
	<div class="section" id="QA_profile1">
        ...
        <div class="optionArea">
            <div class="mOption" style="display: block;">
                ...
                <tbody>
                    ...
                    -->
                    <tr>
                        <th scope="row">롤링선택</th>
                        <td  colspan="3">
                            <select id="s-area-roll-list" name="listCnt" class="fSelect">
							</select>
							<script id="s-temp-roll-list" type="text/x-handlebars-template">
								<option value="">-롤링위치 선택-</option>
								{{#rows}}
								<option value="{{roll_idx}}">{{title}}</option>
								{{/rows}} 
							</script>  							
                        </td>
                    </tr>
		...
		<div class="mButton gCenter">
		    <a href="" id="btn_Search" class="btnSearch"><span>검색</span></a>
		    <a href="" id="btn_Reset" class="btnSearch reset"><span>초기화</span></a>
		</div>
    </div>
	        ...
	        <table border="1" summary="" class="eChkColor">
                ...
                <script id="s-temp-list" type="text/x-handlebars-template">
                    ...
                    <tr class="">
						...
						<td>{{active_yn}}</td>
                        <td><a href="javascript:roll.fn.moveEdit('{{img_idx}}');" class='btnNormal'><span> 조회</span></a></td>
                    </tr>
                    {{/rows}} 
                </script>           
...
<script>
	...
	// 속성 설정
	roll.isLog = true;
	roll.prop["__formUrl"] = "RollingImage_Frm.asp";
	roll.prop["__listUrl"] = "RollingImage_Lst.asp";
	roll.prop['__isGetLoad'] = false;

	// 이벤트 바인딩
	$('#btn_Search').click(roll.fn.searchList);
	$('#changePagesize').change(roll.fn.changePagesize);
	$('#btn_Reset').click(roll.fn.resetSearch);
	$('#btn_Insert').click(roll.fn.moveForm);
	$('#s-area-roll-list').change(roll.fn.changeRolling);
    //--------------------------------------------------------------
	$(document).ready(function () {
        roll.init();
		roll.fn.procListRoll();
    });
