<%
    Class ORD_Product_Cls
        ...
        Public Property Let Op_idx(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("op_idx") = m_cDBMgr.MakeParam("@op_idx", adTinyInt, adParamInput, 0, p_Value)
            End if
        End Property
		...
		Private Sub Init()
            ...
            m_oDicParams.Add "op_idx",          m_cDBMgr.MakeParam("@op_idx",           adTinyInt,      adParamInput,       0,      Null)
        ...
        Public Function Read(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "ord_id", "op_idx", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("ORD_Product_SP_R", arrParams, Nothing)
        ...
        Public Function Update(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "ord_id", "op_idx", "deli_idx")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("ORD_Product_SP_U", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function List(ByRef r_Return, ByRef r_RowTotal)
            ...
            arrKey = Array("RETURN_VALUE", "ord_id", "deli_idx", "keyword", "page_size", "page_count", "sort_cd", "row_total", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("ORD_Product_SP_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
