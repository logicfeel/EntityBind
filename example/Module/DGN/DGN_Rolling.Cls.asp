    ...
    Class BOD_Rolling_Cls
        ...
        Public Property Let Title(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("title") = m_cDBMgr.MakeParam("@title", adVarWChar, adParamInput, 30, p_Value)
            End if
        End Property

        Public Property Let Help(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("help") = m_cDBMgr.MakeParam("@help", adVarWChar, adParamInput, 100, p_Value)
            End if
        End Property

        Public Property Let PcWidth_it(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("pcWidth_it") = m_cDBMgr.MakeParam("@pcWidth_it", adInteger, adParamInput, 0, p_Value)
            End if
        End Property

        Public Property Let PcHeight_it(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("pcHeight_it") = m_cDBMgr.MakeParam("@pcHeight_it", adInteger, adParamInput, 0, p_Value)
            End if
        End Property

        Public Property Let MWidth_it(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("mWidth_it") = m_cDBMgr.MakeParam("@mWidth_it", adInteger, adParamInput, 0, p_Value)
            End if
        End Property

        Public Property Let MHeight_it(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("mHeight_it") = m_cDBMgr.MakeParam("@mHeight_it", adInteger, adParamInput, 0, p_Value)
            End if
        End Property

        Public Property Let Skin_it(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("skin_it") = m_cDBMgr.MakeParam("@skin_it", adInteger, adParamInput, 0, p_Value)
            End if
        End Property

        Public Property Let Roll_idx(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("roll_idx") = m_cDBMgr.MakeParam("@roll_idx", adInteger, adParamInput, 0, p_Value)
            End if
        End Property
		...
		Private Sub Init()
            ...
            m_oDicParams.Add "title",           m_cDBMgr.MakeParam("@title",            adVarWChar, adParamInput,       30,     Null)
            m_oDicParams.Add "help",            m_cDBMgr.MakeParam("@help",             adVarWChar, adParamInput,       100,    Null)
            m_oDicParams.Add "pcWidth_it",      m_cDBMgr.MakeParam("@pcWidth_it",       adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "pcHeight_it",     m_cDBMgr.MakeParam("@pcHeight_it",      adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "mWidth_it",       m_cDBMgr.MakeParam("@mWidth_it",        adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "mHeight_it",      m_cDBMgr.MakeParam("@mHeight_it",       adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "skin_it",         m_cDBMgr.MakeParam("@skin_it",          adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "roll_idx",        m_cDBMgr.MakeParam("@roll_idx",         adInteger,  adParamInput,       0,      Null)
        ...
        Public Function Create(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "title", "help", "pcWidth_it", "pcHeight_it", "mWidth_it", "mHeight_it", "skin_it")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("DGN_Rolling_SP_C", arrParams, Nothing)
        ...
        Public Function Read(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "roll_idx", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("DGN_Rolling_SP_R", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Update(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "roll_idx", "title", "help", "pcWidth_it", "pcHeight_it", "mWidth_it", "mHeight_it", "skin_it")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("DGN_Rolling_SP_U", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Delete(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "roll_idx")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("DGN_Rolling_SP_D", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function List(ByRef r_Return, ByRef r_RowTotal)
            ...
            arrKey = Array("RETURN_VALUE", "page_size", "page_count", "sort_cd", "row_total", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("DGN_Rolling_SP_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
