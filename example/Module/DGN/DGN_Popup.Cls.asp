<%
    Class DGN_Popup_Cls
        ...
        Public Property Let Title(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("title") = m_cDBMgr.MakeParam("@title", adVarWChar, adParamInput, 20, p_Value)
            End if
        End Property

        Public Property Let Type_cd(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("type_cd") = m_cDBMgr.MakeParam("@type_cd", adChar, adParamInput, 1, p_Value)
            End if
        End Property
        ...
        Public Property Let XPosition_cd(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("xPosition_cd") = m_cDBMgr.MakeParam("@xPosition_cd", adChar, adParamInput, 1, p_Value)
            End if
        End Property

        Public Property Let YPosition_cd(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("yPosition_cd") = m_cDBMgr.MakeParam("@yPosition_cd", adChar, adParamInput, 1, p_Value)
            End if
        End Property
        ...
        Public Property Let Active_yn(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("active_yn") = m_cDBMgr.MakeParam("@active_yn", adChar, adParamInput, 1, p_Value)
            End if
        End Property

        Public Property Let Pop_idx(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("pop_idx") = m_cDBMgr.MakeParam("@pop_idx", adInteger, adParamInput, 0, p_Value)
            End if
        End Property
		...
		Private Sub Init()
            ...
            m_oDicParams.Add "title",           m_cDBMgr.MakeParam("@title",            adVarWChar, adParamInput,       20,     Null)
            m_oDicParams.Add "type_cd",         m_cDBMgr.MakeParam("@type_cd",          adChar,     adParamInput,       1,      Null)
            ...
            m_oDicParams.Add "xPosition_cd",    m_cDBMgr.MakeParam("@xPosition_cd",     adChar,     adParamInput,       1,      Null)
            m_oDicParams.Add "yPosition_cd",    m_cDBMgr.MakeParam("@yPosition_cd",     adChar,     adParamInput,       1,      Null)
            ...
            m_oDicParams.Add "active_yn",       m_cDBMgr.MakeParam("@active_yn",        adChar,     adParamInput,       1,      Null)
            m_oDicParams.Add "pop_idx",         m_cDBMgr.MakeParam("@pop_idx",          adInteger,  adParamInput,       0,      Null)
        ...
        Public Function Create(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "title", "type_cd", "contents", "width_it" , "height_it", "xPosition_cd", "yPosition_cd", "begin_dt", "close_dt", "active_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("DGN_Popup_SP_C", arrParams, Nothing)
        ...
        Public Function Read(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "pop_idx", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("DGN_Popup_SP_R", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Update(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "pop_idx", "title", "type_cd", "contents", "width_it" , "height_it", "xPosition_cd", "yPosition_cd", "begin_dt", "close_dt", "active_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("DGN_Popup_SP_U", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Delete(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "pop_idx")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("DGN_Popup_SP_D", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function List(ByRef r_Return, ByRef r_RowTotal)
            ...
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("DGN_Popup_SP_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
		...
		' Return		: RecordSet 객체
		'---------------------------------------------------------------------------
        Public Function ListActive(ByRef r_Return, ByRef r_RowTotal)
            Dim arrKey, arrParams
            Dim oRs

            arrKey = Array("RETURN_VALUE", "type_cd", "page_size", "page_count", "sort_cd", "row_total", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("DGN_Popup_SP_Active_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
            r_RowTotal = m_cDBMgr.GetValue(arrParams, "@row_total") 
		...
		'-------------------------------------
		' 목록 : Dictionary
        Public Function ListActiveDic(ByRef r_Return, ByRef r_RowTotal)
            Dim oDic
            Dim oRs
            
            Set oDic = m_cDBMgr.CRsToDictionary(ListActive(r_Return, r_RowTotal))

            Set ListActiveDic = oDic
        End Function

		'-------------------------------------
		' 목록 : String 객체(XML)
        Public Function ListActiveXml(ByRef r_Return, ByRef r_RowTotal)
            Dim str
            Dim oRs
            
            Xml_yn = "Y"
            Set oRs = ListActive(r_Return, r_RowTotal)
			str = ""
            str = str & "<table return='" & r_Return & "' row_total='" & iRowTotal & "'>"
            If oRs.State = adStateOpen Then str = str & oRs(0)
			str = str & "</table>"
		...
		'-------------------------------------
		' 목록 : String 객체(JSON)
		Public Function ListActiveJson(ByRef r_Return, ByRef r_RowTotal)
            Dim str, oJson
            Dim oRs, strJson : strJson = ""
