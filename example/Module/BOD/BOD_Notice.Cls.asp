<%
    Class BOD_Notice_Cls
        ...
        Public Property Let Top_yn(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("top_yn") = m_cDBMgr.MakeParam("@top_yn", adChar, adParamInput, 1, p_Value)
            End if
        End Property

        Public Property Let Popup_yn(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("popup_yn") = m_cDBMgr.MakeParam("@popup_yn", adChar, adParamInput, 1, p_Value)
            End if
        End Property

        Public Property Let Writer(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("writer") = m_cDBMgr.MakeParam("@writer", adVarWChar, adParamInput, 10, p_Value)
            End if
        End Property
        ...
        Public Property Let Ntc_idx(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("ntc_idx") = m_cDBMgr.MakeParam("@ntc_idx", adInteger, adParamInput, 0, p_Value)
            End if
        End Property

        Public Property Let Division_yn(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("division_yn") = m_cDBMgr.MakeParam("@division_yn", adChar, adParamInput, 1, p_Value)
            End if
        End Property
		...
		Private Sub Init()
            ...
            m_oDicParams.Add "top_yn",          m_cDBMgr.MakeParam("@top_yn",           adChar,     adParamInput,       1,      Null)
            m_oDicParams.Add "popup_yn",        m_cDBMgr.MakeParam("@popup_yn",         adChar,     adParamInput,       1,      Null)
            m_oDicParams.Add "writer",          m_cDBMgr.MakeParam("@writer",           adVarWChar, adParamInput,       10,     Null)
            m_oDicParams.Add "contents",        m_cDBMgr.MakeParam("@contents",         adVarWChar, adParamInput,       2000,   Null)
            m_oDicParams.Add "ntc_idx",         m_cDBMgr.MakeParam("@ntc_idx",          adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "division_yn",     m_cDBMgr.MakeParam("@division_yn",      adChar,     adParamInput,       1,      Null)
        ...
        Public Function Create(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "title", "state_cd", "top_yn", "popup_yn", "writer", "contents")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("BOD_Notice_SP_C", arrParams, Nothing)
        ...
        Public Function Read(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "ntc_idx", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("BOD_Notice_SP_R", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Update(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "ntc_idx", "title", "state_cd", "top_yn", "popup_yn", "writer", "contents")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("BOD_Notice_SP_U", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Delete(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "ntc_idx")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("BOD_Notice_SP_D", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function List(ByRef r_Return, ByRef r_RowTotal)
            ...
            arrKey = Array("RETURN_VALUE", "state_cd", "top_yn", "popup_yn", "division_yn", "keyword", "page_size", "page_count", "sort_cd", "row_total", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("BOD_Notice_SP_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        '     Set oRs = List(r_Return, r_RowTotal)
		' 	str = ""
        '     str = str & "<tables return='" & r_Return & "' >"
        '     str = str & "<table row_total='" & r_RowTotal & "'>"
        '     str = str & oRs(0)
        '     str = str & "</table>"
        '     Set oRs = oRs.NextRecordset     ' 복수 레코드셋 반환시
