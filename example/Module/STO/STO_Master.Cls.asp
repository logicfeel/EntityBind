<%
    Class STO_Master_Cls
        ...
        Public Property Let Sto_id_Output(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("sto_id_Output") = m_cDBMgr.MakeParam("@sto_id", adVarChar, adParamOutput, 6, p_Value)
            End if
        End Property

        Public Property Let StoName(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("stoName") = m_cDBMgr.MakeParam("@stoName", adVarWChar, adParamInput, 20, p_Value)
            End if
        End Property

        Public Property Let Sto_code(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("sto_code") = m_cDBMgr.MakeParam("@sto_code", adChar, adParamInput, 1, p_Value)
            End if
        End Property
		...
		Private Sub Init()
            ...
            m_oDicParams.Add "sto_id_Output",m_cDBMgr.MakeParam("@sto_id",       adVarChar,  adParamOutput,   6,    Null)
            m_oDicParams.Add "stoName",      m_cDBMgr.MakeParam("@stoName",      adVarWChar, adParamInput,    20,   Null)
            m_oDicParams.Add "sto_code",     m_cDBMgr.MakeParam("@sto_code",     adChar,     adParamInput,    1,    Null)
        ...
        ' Description 	: 등록
		'---------------------------------------------------------------------------
        Public Function Create(ByRef r_Return, ByRef r_Sto_id)
            Dim arrKey, arrParams

            arrKey = Array("RETURN_VALUE", "sto_id_Output", "stoName", "sto_code")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("STO_Master_SP_C", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
            r_Sto_id = m_cDBMgr.GetValue(arrParams, "sto_id_Output") 
        ...
        '-------------------------------------
		' 목록 : Dictionary
        Public Function CreateDic(ByRef r_Return, r_Sto_id)
            Dim oDic
            
            Call Create(r_Return, r_Sto_id)
		...
		'-------------------------------------
		' 목록 : String 객체(XML)
        Public Function CreateXml(ByRef r_Return, ByRef r_Sto_id)
            Dim str, oRs
            
            Call Create(r_Return, r_Sto_id)
            str = "<table return='" & r_Return & "' sto_id='" & r_Sto_id & "'></table>"
		...
		'-------------------------------------
		' 목록 : String 객체(XML)
		Public Function CreateJson(ByRef r_Return, ByRef r_Sto_id)
            Dim str, oJson
        ...
        Public Function Read(ByRef r_Return)
            ...
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("STO_Master_SP_R", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Update(ByRef r_Return)
            ...
            ' 파라메터 구성
            arrKey = Array("RETURN_VALUE", "sto_id", "stoName")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("STO_Master_SP_U", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Delete(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "sto_id")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)

            Call m_cDBMgr.ExecuteSp("STO_Master_SP_D", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function List(ByRef r_Return, ByRef r_RowTotal)
            ...
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("STO_Master_SP_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
