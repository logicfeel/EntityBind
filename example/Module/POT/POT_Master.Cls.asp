<%
    Class POT_Master_Cls
        ...
        Public Property Let PointName(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("pointName") = m_cDBMgr.MakeParam("@pointName", adVarWChar, adParamInput, 20, p_Value)
            End if
        End Property

        Public Property Let Method_cd(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("method_cd") = m_cDBMgr.MakeParam("@method_cd", adChar, adParamInput, 1, p_Value)
            End if
        End Property
        ...
        Public Property Let Percent_it(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("percent_it") = m_cDBMgr.MakeParam("@percent_it", adInteger, adParamInput, 0, p_Value)
            End if
        End Property
        ...
        Public Property Let Contents(p_Value) 
            If Not IsEmpty(p_Value) Then
                m_oDicParams("contents") = m_cDBMgr.MakeParam("@contents", adVarWChar, adParamInput, 100, p_Value)
            End if
        End Property
		...
		Private Sub Init()
            ...
            m_oDicParams.Add "pointName",       m_cDBMgr.MakeParam("@pointName",        adVarWChar, adParamInput,       20,     Null)
            m_oDicParams.Add "method_cd",       m_cDBMgr.MakeParam("@method_cd",        adChar,     adParamInput,       1,      Null)
            m_oDicParams.Add "point_it",        m_cDBMgr.MakeParam("@point_it",         adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "percent_it",      m_cDBMgr.MakeParam("@percent_it",       adInteger,  adParamInput,       0,      Null)
            m_oDicParams.Add "use_yn",          m_cDBMgr.MakeParam("@use_yn",           adChar,     adParamInput,       1,      Null)
            m_oDicParams.Add "contents",        m_cDBMgr.MakeParam("@contents",         adVarWChar, adParamInput,       100,    Null)
        ...
        Public Function Create(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "identifier", "pointName", "method_cd", "point_it", "percent_it", "use_yn", "contents")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("POT_Master_SP_C", arrParams, Nothing)
        ...
        Public Function Read(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "pot_id", "xml_yn")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("POT_Master_SP_R", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Update(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "pot_id", "identifier", "pointName", "method_cd", "point_it", "percent_it", "use_yn", "contents")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("POT_Master_SP_U", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function Delete(ByRef r_Return)
            ...
            arrKey = Array("RETURN_VALUE", "pot_id")
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Call m_cDBMgr.ExecuteSp("POT_Master_SP_D", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
        ...
        Public Function List(ByRef r_Return, ByRef r_RowTotal)
            ...
            arrParams = m_cDBMgr.GetDicToArrayParams(m_oDicParams, arrKey)
            Set oRs = m_cDBMgr.ExecuteSpRs("POT_Master_SP_L", arrParams, Nothing)
            r_Return = m_cDBMgr.GetValue(arrParams, "RETURN_VALUE") 
