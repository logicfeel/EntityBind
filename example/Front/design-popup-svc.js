...
(function(global) {
    ...
    var DesignPopupService  = (function (_super) {
        ...
        function DesignPopupService(p_this, p_suffix) {
            ...
            // 모델 속성 설정
            p_this.baseUrl      = "/Front/frt_mod/DGN/Design_Popup.C.asp";
            ...
            this.prop = {
                ...
                type_cd:      { 
                    ...
                    setter: function(val) { 
                        if (val === "B" ) $("input[name=m-type_cd][value=B]").prop("checked", "checked");
                        if (val === "S" ) $("input[name=m-type_cd][value=S]").prop("checked", "checked");
                    }
                },
                width_it:       { selector: { key: "#m-width_it"+ SUFF,         type: "value" } },
                height_it:      { selector: { key: "#m-height_it"+ SUFF,        type: "value" } },
                xPosition_cd:      { 
                    getter: function() { return $("input[name=m-xPosition_cd]:checked").val(); },
                    setter: function(val) { 
                        if (val === "R" ) $("input[name=m-xPosition_cd][value=R]").prop("checked", "checked");
                        if (val === "C" ) $("input[name=m-xPosition_cd][value=C]").prop("checked", "checked");
                        if (val === "L" ) $("input[name=m-xPosition_cd][value=L]").prop("checked", "checked");
                    }
                },
                yPosition_cd:      { 
                    getter: function() { return $("input[name=m-yPosition_cd]:checked").val(); },
                    setter: function(val) { 
                        if (val === "T" ) $("input[name=m-yPosition_cd][value=T]").prop("checked", "checked");
                        if (val === "M" ) $("input[name=m-yPosition_cd][value=M]").prop("checked", "checked");
                        if (val === "B" ) $("input[name=m-yPosition_cd][value=B]").prop("checked", "checked");
                    }
                },
            };
            // mapping 설정
            this.mapping = {
                ...
                type_cd:        { read:     "output",   create:     "bind",            update:  "bind" },
        ...
        // 데코레이션 메소드
        DesignPopupService.prototype.preRegister = function(p_this) {
        ...
        DesignPopupService.prototype.preCheck = function(p_this) {
        ...
        DesignPopupService.prototype.preReady = function(p_this) {
            ...
            if (p_this.prop["__mode"] === "CREATE") {
                ...
                p_this.items["pop_idx"].value = ParamGet2JSON(location.href).pop_idx;
            }  
        };
