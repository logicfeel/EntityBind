...
var BoardService = (function (_super) {
    function BoardServie(p_suffix) {
        ...
        this.cbBaseValid: function(p_cmd) { /*..*/ };
        
        this.onExecute: function(p_cmd) { /*..*/ };
        ...
        this.prop = {
            ...
            //---------------------------------------------------------------------------
            // 방법 1> :: getter + onChange 방식  | selector >> X, __value >> X
            _sumTotal:  { 
                getter:     function(val) { return _this.items['price'].value * 10;  },
            ...
            //---------------------------------------------------------------------------
            // 방법 2> :: setter + selector 방식 | selector >> O, __value >> O
            _sumTotal:  { 
                selector:   { key: '#s-sum'+ _SUFF,    type: 'text' },
                filter:     function(val) { return comma(val); },  // get 필터
                setter:     function(val) { return _this.items['price'].value * 10;  },   // 값계산 : setter 에서 r eturn 을 수행
             },
            price:     {  
                selector:   { key: '#s-price'+ _SUFF,    type: 'text' },
                filter:     function(val) { return comma(val); },                       // set 필터
                onChange:   function(val) { _this.items['_sumTotal'].value = 0; }       // 임의값 삽입(이벤트 발생)
             },
            //---------------------------------------------------------------------------
            // mapping
            cmd:        '',
...
board.cbBaseValid = function(p_cmd) { /*..*/ };

board.onExecute = function(p_cmd) { /*..*/ };
