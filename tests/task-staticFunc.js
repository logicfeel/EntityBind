...
function IBinder() {
    ...
    this.preRegister;
    this.cbValid;
    this.preReady;   // 이부분은 혼선을 피하기 위해서 인터페이스에서 빼는거 적당할듯 => 덮어써서 자용하면됨 (자동화의 이득)
}
...
function Page() {
    ...
    this.prop = {                                          // idx 같이 고유 명칭이므로 의존성이 높음
        cmd: "INSERT",
        idx: 1,                 // 확인필요
...
ㄴ
     ...
     * @param {*} bindModel 
     */
    this.preRegister = function(bindModel) {
    ...
    //====================================
    // 의존성이 섞여 있는 부분
    this.preReady = function(bindModel) {           // 의존성이 섞여 있음 !!! => 파라메터로 해결, 선택적으로 사용
        // 재개(대기) 
...
bm.preReady = function() {                  // 의존성이 섞여 있음 !!! => 파라메터로 해결
    // 시작시 처리할 부분이 있으면 추가함
};
bm.preRegister = function() {                // read, 이런 부분을 모를 경우 오버라이딩함

}

Item("addr");
addr.refValue = bm.prop["addr"];           // 상단에 참조를 정의해두면 이런식으로 쉽게 사용함
